...
| 項目 | 内容 | 区分 |
| --- | --- | --- |
| 内容 | タイトルと**同じ**（一般：言語 ID・FPS 三択・非アクティブ系／音量／キー表示のみ） | 確定 |
| 永続化 | 既存 `SoundSettingsManager`（変更即保存）。FPS は `frameRateMode` | 確定 |
...
| 日付 | 内容 |
| --- | --- |
| 2026-09-26 | §4.1：FPS 三択 UI を一般タブに追加（タイトル共通プレハブ） |
| 2026-09-26 | §4.1：FPS は API のみ・UI 後続を明記 |
