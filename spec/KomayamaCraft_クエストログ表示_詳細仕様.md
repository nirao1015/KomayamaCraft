...
| イベント中 | 他の `Layer_System` HUD と同じ表示／非表示方針に従う | 確定（個別例外なし） |
...
| 幅 | 仮置き相当 **約 400**。高さだけ可変 | 提案（現行たたき台） |
| 最大高さ | あり。中身がこれを超えたら**スクロール** | 確定 |
| 最大高さの数値 | 基準解像度 1080 に対し **約 540**（画面半分）を初期 | 提案（Inspector で調整可） |

### 5.2 高さの決まり方

1. **最小化中**：ボードは出さない。最小化ボタンのサイズのみ（左上アンカー付近に固定配置）。
2. **展開中・内容が最大高さ未満**：リスト preferred height にボードが追従（9-slice）。
3. **展開中・内容が最大高さ以上**：ボード高さは最大高さで固定。`QuestList` を ScrollRect で縦スクロール。

内容は TMP の preferred height に追従させる。フレームはスクロール領域を包む。
