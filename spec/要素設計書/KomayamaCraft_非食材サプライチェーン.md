...
これとは別に、浜辺のワールド物体**蓄電クラゲ**がおり、そこから落ちる**蓄電触腕**（`item.volt_jelly`）を加工施設の燃料として使う。蓄電触腕は加工素材には数えず、拾って作業台へ右クリック投入する。
...
浜辺に定着する、体内に電荷を蓄える原生生物（ワールド物体。シーンに1体・`Layer_Objects`）。
**鉄鱗獣などのクリック採集オブジェクトとは別系統**（`KomayamaResourceNode` を使わない。専用の自動ドロップ実装）。
クリックは現状何もしない。範囲内へ**蓄電触腕**を自動ドロップする（範囲内上限・分布は指示差分 §7.1.1）。

**蓄電触腕**は拾う／落とせる通常アイテムで、加工施設の燃料枠へ右クリック投入する（`item.volt_jelly`）。クラフト用の生素材ではなく燃料枠として扱う。
...
強い磁性を持つ角を備えた原生生物。
`Layer_Objects/磁角獣`（`place_磁角獣`）。クリック採集（`KomayamaResourceNode`／鉄鱗獣と同系統）。
...
`Layer_Objects/星紋結晶`（`place_星紋結晶`）。クリック採集。
...
空間の揺らぎが固定化した薄片状物質。
`Layer_Objects/位相断層`（`place_位相断層`）。クリック採集。
...
`Layer_Objects/軌道繭`（`place_軌道繭`）。クリック採集（Tier 4 詳細も参照）。
...
設備は稼働時に**蓄電触腕を燃料として消費する**。
蓄電触腕は事前加工せず、そのまま燃料枠へ投入する。
...
**鱗圧延作業台（現行：基礎加工台ほか全加工施設）は各加工時に蓄電触腕を燃料として別途消費する。蓄電触腕は加工せず、そのまま投入する。**
...
蓄電触腕は加工施設を動かす限り継続して必要になる（供給源は浜辺の蓄電クラゲ）。
そのため新Tier解放後も、初期の燃料供給地点と搬送・投入の自動化を強化する意味が残る。
...
|---|---|---|---|
| 鱗圧延作業台 | 鉄鱗、鉄鱗板、燃料の蓄電触腕 | 板、リベット、放熱板、フレーム | 蓄電触腕の燃料供給を切らさないか |
...
| 対象 | 操作 | 消費 | 結果 |
|---|---|---|---|
| 原生生物・植物・鉱物 | クリック | クリック回数・時間 | 素材をドロップ（蓄電クラゲ本体は現状 no-op。触腕は自動ドロップ） |
| 地面の素材 | クリック／ドラッグ回収 | なし | 所持品へ加算（蓄電触腕含む） |
| 作業台建設 | 設備を選択して配置 | 建設素材 | 作業台設置 |
| 鱗圧延作業台 | レシピを選び素材と燃料を投入 | 素材＋蓄電触腕 | 指定部品を確定生成 |
...
- 鉄鱗を集める
- 熱嚢を集める
- 浜辺の蓄電クラゲから落ちる蓄電触腕を拾う
- 蓄電触腕を燃料にして鉄鱗を機械加工する
- 熱嚢を共生組立槽へ投入し、特殊な実を培養する
- 船体と補助動力を復旧する

序盤の中心は**加工用の蓄電触腕を切らさず、鉄鱗加工と共生培養を並行させること**。
...
| 項目 | 現在案 |
|---|---|
| 加工施設の燃料 | 蓄電触腕（`item.volt_jelly`）。浜辺の蓄電クラゲから自動ドロップ。加工せず投入 |
| 共生組立槽の抽選入力 | 熱嚢 |
