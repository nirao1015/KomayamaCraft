...
{
    /// <summary>
    /// WorkUpgradeSt04 配信環境改善仕事場（work_upgrade_st04_spec.md）。
    /// </summary>
    public sealed class WorkUpgradeSt04Controller : MonoBehaviour, IWorkplaceTarget
    {
        public const int MaxStreamEnvironmentCompletions = int.MaxValue;
        public const long BasePrice = 99800L;
        public const double PriceMultiplier = 6.40d;
        ...
        [SerializeField] private WorkUpgradeSt04AcceptEffect acceptEffect;
        ...
        private static bool hasAutoConfiguredSceneWorkUpgradeSt04;
        ...
        public static void EnsureSceneWorkUpgradeSt04Exists()
        {
            if (hasAutoConfiguredSceneWorkUpgradeSt04)
            {
                return;
            }

            hasAutoConfiguredSceneWorkUpgradeSt04 = true;
            EnsureOneSceneWorkUpgradeSt04("PanelCanvas/PanelObject/SidePanelObject/SidePanelView/WorkUpgradeSt04");
            EnsureOneSceneWorkUpgradeSt04("WorkUpgradeSt04");
        }

        private static void EnsureOneSceneWorkUpgradeSt04(string objectPath)
        {
            ...
            if (go.GetComponent<WorkUpgradeSt04Controller>() != null)
            {
                return;
            }

            go.AddComponent<WorkUpgradeSt04Controller>();
        }
        ...
        private void Awake()
        {
            ...
            {
                acceptEffect = GetComponent<WorkUpgradeSt04AcceptEffect>();
            }
        ...
        public void OnItemDropped(DraggableItemController item)
        {
            ...
            if (!GameManager.Instance.RequestMoneyDelta(-price, "WorkUpgradeSt04 placement", "WorkUpgradeSt04", -1))
            {
                TryPlayPurchaseFailureSe();
                return;
            }
            ...
            {
                Debug.LogError("[WorkUpgradeSt04] Accepted sprite is null.");
                return;
            }
        ...
        private IEnumerator RunAcceptFlow(Sprite acceptedSprite)
        {
            ...
            if (acceptEffect != null)
            {
                ...
                {
                    Debug.LogError("[WorkUpgradeSt04] AcceptedItemView is missing or sprite not applied after accept effect.");
        ...
        private int GetPriceLevelForNextPlacement()
        {
            ...
            return Mathf.Max(0, UpgradesManager.Instance.GetStreamEnvironmentUpgradeLevel());
        }
        ...
        private bool ApplyAcceptedItemViewCore(Sprite acceptedSprite)
        {
            if (acceptedItemView == null)
            {
                Debug.LogError("[WorkUpgradeSt04] AcceptedItemView is missing.");
                return false;
            }
        ...
        private void EndWorkSession()
        {
            if (UpgradesManager.Instance != null)
            {
                UpgradesManager.Instance.ApplySt04WorkComplete();
            }
        ...
        private void RespawnMailCharaAfterWorkExit()
        {
            ...
            {
                Debug.LogWarning("[WorkUpgradeSt04] Work exit respawn skipped: ItemSpawnController or ItemCanvas not found.");
                return;
            }

            if (!TryGetSpawnPointCenterSelfInItemCanvas(cachedSpawnController.ItemCanvas, out Vector2 centerAnchored))
            {
                Debug.LogWarning("[WorkUpgradeSt04] Work exit respawn skipped: could not resolve center in ItemCanvas.");
                return;
            }
            ...
            {
                Debug.LogWarning("[WorkUpgradeSt04] ItemMailChara work-exit respawn produced no DraggableItemController on instance.");
            }
        }
        ...
        private static void LogWorkExitSpawnFailure(string itemLabel)
        {
            ...
            {
                Debug.LogWarning($"[WorkUpgradeSt04] {itemLabel} work-exit respawn skipped: GameManager fatal state (spawn blocked).");
            }
            else
            {
                Debug.LogWarning($"[WorkUpgradeSt04] {itemLabel} work-exit respawn failed: TrySpawnItem returned false (prefab / canvas / controller).");
            }
        }
