...
{
    /// <summary>
    /// WorkUpgradeSt02 トレンド力仕事場（work_upgrade_st02_spec.md）。
    /// </summary>
    public sealed class WorkUpgradeSt02Controller : MonoBehaviour, IWorkplaceTarget
    {
        public const int MaxTrendPowerCompletions = int.MaxValue;
        public const long BasePrice = 120000L;
        public const double PriceMultiplier = 6.20d;
        ...
        [SerializeField] private WorkUpgradeSt02AcceptEffect acceptEffect;
        ...
        private static bool hasAutoConfiguredSceneWorkUpgradeSt02;
        ...
        public static void EnsureSceneWorkUpgradeSt02Exists()
        {
            if (hasAutoConfiguredSceneWorkUpgradeSt02)
            {
                return;
            }

            hasAutoConfiguredSceneWorkUpgradeSt02 = true;
            EnsureOneSceneWorkUpgradeSt02("PanelCanvas/PanelObject/SidePanelObject/SidePanelView/WorkUpgradeSt02");
            EnsureOneSceneWorkUpgradeSt02("WorkUpgradeSt02");
        }

        private static void EnsureOneSceneWorkUpgradeSt02(string objectPath)
        {
            ...
            if (go.GetComponent<WorkUpgradeSt02Controller>() != null)
            {
                return;
            }

            go.AddComponent<WorkUpgradeSt02Controller>();
        }
        ...
        private void Awake()
        {
            ...
            {
                acceptEffect = GetComponent<WorkUpgradeSt02AcceptEffect>();
            }
        ...
        public void OnItemDropped(DraggableItemController item)
        {
            ...
            if (!GameManager.Instance.RequestMoneyDelta(-price, "WorkUpgradeSt02 placement", "WorkUpgradeSt02", -1))
            {
                TryPlayPurchaseFailureSe();
                return;
            }
            ...
            {
                Debug.LogError("[WorkUpgradeSt02] Accepted sprite is null.");
                return;
            }
        ...
        private IEnumerator RunAcceptFlow(Sprite acceptedSprite)
        {
            ...
            if (acceptEffect != null)
            {
                ...
                {
                    Debug.LogError("[WorkUpgradeSt02] AcceptedItemView is missing or sprite not applied after accept effect.");
        ...
        private int GetPriceLevelForNextPlacement()
        {
            ...
            return Mathf.Max(0, UpgradesManager.Instance.GetTrendPowerUpgradeLevel());
        }
        ...
        private bool ApplyAcceptedItemViewCore(Sprite acceptedSprite)
        {
            if (acceptedItemView == null)
            {
                Debug.LogError("[WorkUpgradeSt02] AcceptedItemView is missing.");
                return false;
            }
        ...
        private void EndWorkSession()
        {
            if (UpgradesManager.Instance != null)
            {
                UpgradesManager.Instance.ApplySt02WorkComplete();
            }
        ...
        private void RespawnMailCharaAfterWorkExit()
        {
            ...
            {
                Debug.LogWarning("[WorkUpgradeSt02] Work exit respawn skipped: ItemSpawnController or ItemCanvas not found.");
                return;
            }

            if (!TryGetSpawnPointCenterSelfInItemCanvas(cachedSpawnController.ItemCanvas, out Vector2 centerAnchored))
            {
                Debug.LogWarning("[WorkUpgradeSt02] Work exit respawn skipped: could not resolve center in ItemCanvas.");
                return;
            }
            ...
            {
                Debug.LogWarning("[WorkUpgradeSt02] ItemMailChara work-exit respawn produced no DraggableItemController on instance.");
            }
        }
        ...
        private static void LogWorkExitSpawnFailure(string itemLabel)
        {
            ...
            {
                Debug.LogWarning($"[WorkUpgradeSt02] {itemLabel} work-exit respawn skipped: GameManager fatal state (spawn blocked).");
            }
            else
            {
                Debug.LogWarning($"[WorkUpgradeSt02] {itemLabel} work-exit respawn failed: TrySpawnItem returned false (prefab / canvas / controller).");
            }
        }
