...
{
    /// <summary>
    /// WorkUpgradeSt03 SEO力仕事場（work_upgrade_st03_spec.md）。
    /// </summary>
    public sealed class WorkUpgradeSt03Controller : MonoBehaviour, IWorkplaceTarget
    {
        public const int MaxSeoPowerCompletions = int.MaxValue;
        public const long BasePrice = 90000L;
        public const double PriceMultiplier = 5.20d;
        ...
        [SerializeField] private WorkUpgradeSt03AcceptEffect acceptEffect;
        ...
        private static bool hasAutoConfiguredSceneWorkUpgradeSt03;
        ...
        private Transform invalidViewRoot;
        ...
        public static void EnsureSceneWorkUpgradeSt03Exists()
        {
            if (hasAutoConfiguredSceneWorkUpgradeSt03)
            {
                return;
            }

            hasAutoConfiguredSceneWorkUpgradeSt03 = true;
            EnsureOneSceneWorkUpgradeSt03("PanelCanvas/PanelObject/SidePanelObject/SidePanelView/WorkUpgradeSt03");
            EnsureOneSceneWorkUpgradeSt03("WorkUpgradeSt03");
        }

        private static void EnsureOneSceneWorkUpgradeSt03(string objectPath)
        {
            ...
            if (go.GetComponent<WorkUpgradeSt03Controller>() != null)
            {
                return;
            }

            go.AddComponent<WorkUpgradeSt03Controller>();
        }
        ...
        private void Awake()
        {
            ...
            {
                acceptEffect = GetComponent<WorkUpgradeSt03AcceptEffect>();
            }
            ...
            ResolveAcceptedItemViewReferences();
            ...
            ResolveInvalidViewRootIfNeeded();
        ...
        public void OnItemDropped(DraggableItemController item)
        {
            ...
            if (!GameManager.Instance.RequestMoneyDelta(-price, "WorkUpgradeSt03 placement", "WorkUpgradeSt03", -1))
            {
                TryPlayPurchaseFailureSe();
                return;
            }
            ...
            {
                Debug.LogError("[WorkUpgradeSt03] Accepted sprite is null.");
                return;
            }
        ...
        private IEnumerator RunAcceptFlow(Sprite acceptedSprite)
        {
            ...
            if (acceptEffect != null)
            {
                ...
                {
                    Debug.LogError("[WorkUpgradeSt03] AcceptedItemView is missing or sprite not applied after accept effect.");
        ...
        private int GetPriceLevelForNextPlacement()
        {
            ...
            return Mathf.Max(0, UpgradesManager.Instance.GetSeoPowerUpgradeLevel());
        }
        ...
        private void ResolveInvalidViewRootIfNeeded()
        {
            if (invalidViewRoot != null)
            {
                return;
            }

            invalidViewRoot = transform.Find("InvalidView");
        }

        /// <summary>
        /// Inspector で InvalidView の Image を誤参照している場合、子 AcceptedItemView へ補正する。
        /// </summary>
        private void ResolveAcceptedItemViewReferences()
        {
        ...
        /// <summary>
        /// InvalidView 自身の Image を acceptedItemView にしているか（誤配線検出用）。
        /// </summary>
        private bool AcceptedItemViewIsUnderInvalidView()
        {
            ResolveInvalidViewRootIfNeeded();
            ...
            Transform t = acceptedItemView.transform;
            return t == invalidViewRoot || t.IsChildOf(invalidViewRoot);
        }
        ...
        private bool ApplyAcceptedItemViewCore(Sprite acceptedSprite)
        {
            if (acceptedItemView == null)
            {
                Debug.LogError("[WorkUpgradeSt03] AcceptedItemView is missing.");
                return false;
            }
        ...
        private void RestoreAcceptedItemPresentationFromState()
        {
            ...
            ResolveAcceptedItemViewReferences();
            if (acceptedItemView != null && !AcceptedItemViewIsUnderInvalidView())
            {
        ...
        private void EndWorkSession()
        {
            if (UpgradesManager.Instance != null)
            {
                UpgradesManager.Instance.ApplySt03WorkComplete();
            }
        ...
        private void RespawnMailCharaAfterWorkExit()
        {
            ...
            {
                Debug.LogWarning("[WorkUpgradeSt03] Work exit respawn skipped: ItemSpawnController or ItemCanvas not found.");
                return;
            }

            if (!TryGetSpawnPointCenterSelfInItemCanvas(cachedSpawnController.ItemCanvas, out Vector2 centerAnchored))
            {
                Debug.LogWarning("[WorkUpgradeSt03] Work exit respawn skipped: could not resolve center in ItemCanvas.");
                return;
            }
            ...
            {
                Debug.LogWarning("[WorkUpgradeSt03] ItemMailChara work-exit respawn produced no DraggableItemController on instance.");
            }
        }
        ...
        private static void LogWorkExitSpawnFailure(string itemLabel)
        {
            ...
            {
                Debug.LogWarning($"[WorkUpgradeSt03] {itemLabel} work-exit respawn skipped: GameManager fatal state (spawn blocked).");
            }
            else
            {
                Debug.LogWarning($"[WorkUpgradeSt03] {itemLabel} work-exit respawn failed: TrySpawnItem returned false (prefab / canvas / controller).");
            }
        }
